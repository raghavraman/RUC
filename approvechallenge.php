...
<head>
    ...
    $(document).ready(function() {
        ...
        var challengeContent ='<div class="post-footer-comment-wrapper"><div class="card"><div class="media"><div class="media-left"><a href="#"><img class="media-object photo-profile" src=<?php echo $_SESSION['user_photo'];?> width="32" height="32" alt="..."></a></div><div class="media-body"><h2 class="media-heading"><?php echo $_SESSION['user_name'];?></h2></div><div <div class="post-body"><label>Challenge Description</label><p id="postDescription">CHALLENGETEXT</p><label>Challenge Points</label><p id="postDescription">CHALLENGEPOINTS &nbsp;Point(s)</p><label>Time to Complete Challange</label><p id="postDescription">CHALLENGEDURATION&nbsp;Day(s)</p><button id="saveDetails" type="button" class="pull-right btn btn-primary commentsBTn" onclick="acceptChallenge(CHALLENGESAVEID);">Approve Challenge</button><button style="margin-right: 7px;" id="saveDetails" type="button" class="pull-right btn btn-primary commentsBTn" onclick="disapproveChallenge(CHALLENGESAVEID1);">Disapprove Challenge</button></div></div></div></div>';

         var challengeContentwithoutbutton ='<div class="post-footer-comment-wrapper"><div class="card"><div class="media"><div class="media-left"><a href="#"><img class="media-object photo-profile" src=<?php echo $_SESSION['user_photo'];?> width="32" height="32" alt="..."></a></div><div class="media-body"><h2 class="media-heading"><?php echo $_SESSION['user_name'];?></h2></div><div <div class="post-body"><label>Challenge Description</label><p id="postDescription">CHALLENGETEXT</p><label>Challenge Points</label><p id="postDescription">CHALLENGEPOINTS &nbsp;Point(s)</p><label>Time to Complete Challange</label><p id="postDescription">CHALLENGEDURATION&nbsp;Day(s)</p></div></div></div></div>';

    function getdetails(){
        $(document).ready(function() {
            $.ajax({
              type: "POST",
              url: "activeChallenges.php",
              data: {},
              success: function(response){
                response=JSON.parse(response);
                console.log(response);
                var challenges =[];
                challenges=response["all_challenges"];
                 challenges.forEach(function(response){
                    var t;
                    if(response.is_accepted==1 && response.active==1){
                        t=challengeContent.replace("CHALLENGEDURATION",response.duration)
                        ...
                        t=t.replace("CHALLENGESAVEID1",response.challenge_id);
                        $('#challengeList').append('<li id="'+response.challenge_id+'">'+t+'</li>');
                    }else{
                        t=challengeContentwithoutbutton.replace("CHALLENGEDURATION",response.duration)
                        t=t.replace("CHALLENGEPOINTS",response.points);
                        t=t.replace("CHALLENGETEXT",response.desc_text);
                        $('#challengeList').append('<li id="'+response.challenge_id+'">'+t+'</li>');
                    }      
    ...
    function acceptChallenge(id) {
        var jsonobj = {"challengeId": id}
        $.ajax({
            type: "POST",
            url: "approveChallenges.php",
        ...
        function disapproveChallenge(id) {
            var jsonobj = {"challengeId": id}
            $.ajax({
                type: "POST",
                url: "disapproveChallenges.php",
                data: { challenge_info: JSON.stringify(jsonobj)},
                success: function(response){
                    console.log(response);
                    response=JSON.parse(response);
    ...
    function createChallenge(){
        if(!$("#challenge_desc").val()){
            alert("Please enter a Description for the challenge");
            return;
        }
               ...
               <div class="user-dashboard">
                     <div class="challengenavbar">
                      <ul>
                        ...
                        <li class="activechallenge  push"><a href="approvechallenge.php">My Posted Challanges</a></li>
                        <li class="push"><a href="systemchallenge.php">System Challenges</a></li>
