...
if ($result->num_rows > 0) {
    ...
    if ($conn->query($sql) === TRUE) {
		...
		$bonus = ORM::for_table('user_bonuses')->create();
		$bonus->user_id = $get_id_query->id();
		$bonus->save();
		...
		if($get_id_query)
		{
			...
			//Give 5 points to user who referred
			$stmt = $conn->prepare("UPDATE `user_points` SET `points`= `points`+ 5 WHERE `user_id` = ?");
			$stmt->bind_param("i", $refered_by);
			$stmt->execute();
            ...
            $stmt = $conn->prepare("INSERT INTO user_badges(badge_id, user_id) VALUES (?, ?)");
            $badge_id = 7;


            $stmt->bind_param("ii", $badge_id,$_SESSION['user_id']);
