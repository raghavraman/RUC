...
ORDER BY created_at DESC LIMIT 75)')->raw_query('
 ...
 x.user_id = y.user_id and x.user_id = z.user_id 
ORDER BY x.created_at DESC LIMIT 75) a LEFT JOIN post_reactions_aggr b ON
	a.post_id = b.post_id 
	LEFT OUTER JOIN comments_aggr c ON
	a.post_id = c.post_id order by a.created_at DESC', array('userid' => $user_id_temp))->find_many();
    ...
    foreach ($get_posts as $post_info) {
		...
		$post_to_send->timestamp = $post_info->created_at;
...
function put_post_reaction_data($obj_temp, $user_id_temp, $host_t, $user_t, $pass_t, $database_t){
	...
	$date = date('Y-m-d H:i:s');
	$update_post = ORM::for_table('posts')->where('post_id', $obj_temp->post_id)->find_one();
