...
function get_data($obj_temp, $user_id_temp){
    ...
    if($get_id_query){
        ...
        if($get_id_query_points){
			...
			$obj_to_send->no_of_referrals_remaining = $get_id_query_points->no_of_referrals_remaining;
...
function generate_code($obj_temp, $user_id_temp,$host_t, $user_t, $pass_t, $database_t){
	...
	if($person){
		...
		if($generated_for_check){
            ...
            $stmt = $mysqli->prepare("SELECT user_id FROM user_points WHERE user_id = ? AND no_of_referrals_remaining > 0 ");
            ...
            if (!$stmt->execute()) {
                ...
                else
                {
                    ...
                    if(!$mysqli->connect_error)
                    {
                        $stmt = $mysqli->prepare("INSERT INTO signup_token (token,genreated_by,generated_for) VALUES (?,?,?)");
                        $stmt->bind_param("sis", $string, $user_id_temp, $generated_for_email_id);
                        $stmt->execute();
                        $stmt->close();

                        $stmt = $mysqli->prepare("UPDATE user_points SET no_of_referrals_remaining = no_of_referrals_remaining-1 WHERE user_id = ?");
                        $stmt->bind_param("i", $user_id_temp);
                        $stmt->execute();
