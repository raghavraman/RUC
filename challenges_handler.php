...
function put_accept_challenges_data($obj_temp, $user_id_temp, $host_t, $user_t, $pass_t, $database_t){
	...
	if($get_status){
		...
		if (!$stmt->execute()) {
			...
			$now = time();
			$ten_minutes = $now + ($get_status->duration * 60 * 60 * 24);
			...
			$selected_challenge = ORM::for_table('user_challenges')->where('challenge_id', $obj_temp->challenge_id)->find_one();
			$accepted_query = ORM::for_table('users')->where('user_id', $user_id_temp)->find_one();
			$challenge_success_post = ORM::for_table('posts')->create();
			$challenge_success_post->user_id = $selected_challenge->creator_user_id;
			$challenge_success_post->desc_text = $accepted_query->user_name ." accepted my challenge";
